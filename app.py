...
ADMIN_LINE_NAMES = [x.strip() for x in os.getenv("ADMIN_LINE_NAMES", "").split(",") if x.strip()]
...
def reply(reply_token: str, text: str):
    ...
    with ApiClient(configuration) as api_client:
        MessagingApi(api_client).reply_message(ReplyMessageRequest(reply_token=reply_token, messages=[TextMessage(text=text[:4900])]))
...
def is_admin(display_name: str) -> bool:
    return True if not ADMIN_LINE_NAMES else display_name in ADMIN_LINE_NAMES
...
def on_text(event):
    ...
    if text == "結束" and is_admin(name):
        orders.stop()
        return
    if not orders.active or not orders.menu:
        return
    result = ai.parse_chat(message=text, user_name=name, menu=orders.menu, current_orders=orders.public_orders(), last_order_user=orders.last_order_user_name)
...
def on_image(event):
    try:
        ...
        if image_type == "menu" and confidence >= 0.65 and menu:
            # 只要判斷是菜單圖片，就直接清空舊訂單，開始新一輪
            orders.reset(menu=menu, active=True)
            print("MENU STARTED:", orders.menu_names())
        else:
            print("IMAGE IGNORED:", result)
    except Exception as e:
